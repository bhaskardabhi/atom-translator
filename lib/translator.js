...
export default {
  ...
  config: {
      source: {
          ...
          default: 'Google Translation',
          enum: ['Yandex','Google Translation'],
          order: 1
      },
      lang: {
         title: 'Translate To Language',
         description: 'Specify which language to translate by default (e.g. `ru` or `de`). Russian will be default',
         type: 'string',
         default: 'ru',
         order: 2
     },
     key: {
         ...
         description: 'Set Translation API Key Provided By your translation service provider. https://github.com/eDubrovsky/atom-translator#-get-api-key-at',
         type: 'string',
         default: '',
         order: 3
     },
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
        'atom-translator-tooltips:toggle': () => this.toggle()
    }));
  },
  ...
  toggle() {
     ...
     if(editor && editor.getLastSelection() && editor.getLastSelection().getText()){
         ...
         this.translationProvider.translate(this.translationProvider.getDefaultTraslationLang(),selection.getText(), function (word) {
            // add tooltips
            if (atom.config.get('atom-translator-tooltips.enableTooltips')){
                const editorView = atom.views.getView(editor);
                const cursorPos = editorView.querySelector('.cursor');

                tooltips = atom.tooltips.add(cursorPos, {title: word, trigger: "manual", placement: "bottom",
                ...
                atom.workspace.onDidChangeActivePaneItem(({Value}) => { tooltips.dispose(); });
                editor.onDidChangeSelectionRange(({Value}) => { tooltips.dispose(); });
            } else selection.insertText(word);
         });
     }
 },
};
