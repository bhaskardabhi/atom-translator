...
export default {
  ...
  config: {
     ...
     enableTooltips: {
         title: "Display Translation in Tooltip",
         type: "boolean",
         description: 'If enabled, text will be displayed at tooltip',
         default: false,
         order: 4
     }
   },
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
        'atom-translator:toggleToolTip': () => this.toggle(true)
    }));
  },
  ...
  toggle(toggleTooltip) {
     ...
     if(editor && editor.getLastSelection() && editor.getLastSelection().getText()){
         ...
         this.translationProvider.translate(this.translationProvider.getDefaultTraslationLang(),selection.getText(), function (word) {
            // add tooltips
            if (atom.config.get('atom-translator.enableTooltips') || toggleTooltip) {
                if (this.tooltips) {
